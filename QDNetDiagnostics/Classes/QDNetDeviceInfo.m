...
{
    NSArray* ipArray = [QDNetDeviceInfo addressesForHostname:hostName];
    if (ipArray != nil && ipArray.count > 0) {
        return ipArray[0];
    } else {
        return  nil;
    }
...
//            NSLog(@"ifa_name===%@",[NSString stringWithUTF8String:temp_addr->ifa_name]);
            ...
            if ([[NSString stringWithUTF8String:temp_addr->ifa_name] isEqualToString:@"en0"] || [[NSString stringWithUTF8String:temp_addr->ifa_name] isEqualToString:@"pdp_ip0"])
            {
                ...
                if (temp_addr->ifa_addr->sa_family == AF_INET){
                    ...
                    NSString* address = [self formatIPV4Address:((struct sockaddr_in *)temp_addr->ifa_addr)->sin_addr];
                    //以FE80开始的地址是单播地址
                    ...
                    NSString* address = [self formatIPV6Address:((struct sockaddr_in6 *)temp_addr->ifa_addr)->sin6_addr];
                    if (address && ![address isEqualToString:@""] && ![address.uppercaseString hasPrefix:@"FE80"]) {
                      [addressArray addObject:address];
                    }
                }
...
+ (NSArray *)getGatewayIPAddress {
    ...
    if (gatewayIPV6 != nil) {
        [address addObject:gatewayIPV6];
    }
    
    if (gatewayIPV4 != nil) {
        [address addObject:gatewayIPV4];
    }
